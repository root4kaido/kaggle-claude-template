...
3. **スクレイピングスクリプトの確認/作成**:
   - `docs/survey/discussion/` にスクレイピングスクリプトがあるか確認
   - なければ `scrape_with_playwright.py` と `scrape_discussion_details.py` を作成

4. **データ取得**:
   ```bash
   cd docs/survey/discussion
...
6. **結果の整理**:
   - `docs/survey/discussion/snapshot_YYYYMMDD.json` にスナップショット保存
   - `docs/survey/discussion/activity_summary.md` に活動分析レポートを更新
