...
**精度を上げることを第一に考え、工数やコスパは気にしないでください。**

**1つに絞らず、なるべく多様なアイデア候補を複数出すこと。** 似た発想に偏らず、切り口（前処理・モデル・損失・データ・後処理・アンサンブル等）を変えて幅を持たせる。
...
- /tmpにファイルを作らない
- 学習logをteeで出力しない（標準出力のリダイレクトでログを二重化しない）
- `CUDA_VISIBLE_DEVICES=1` 等のGPU指定を勝手につけない
